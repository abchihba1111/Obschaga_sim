...
// Дверь должна иметь тег Door и компонент Rigidbody
...
public class Door : MonoBehaviour
{
    ...
    private bool open_close_ON; // открывается или закрывается в данный момент (чтобы постоянное в апрейте не работало)
    ...
    void OnMouseDown() // можно проверять дверь лучём или другим способом
    {
        if (gameObject.tag == "Door")
        {
            if (door_handle)
            {
                if (handle_axis == handle_axis_ENUM.X) door_handle.transform.localRotation = Quaternion.Euler(handle_rot_angle, 0, 0);
                else if (handle_axis == handle_axis_ENUM.Y) door_handle.transform.localRotation = Quaternion.Euler(0, handle_rot_angle, 0);
                else if (handle_axis == handle_axis_ENUM.Z) door_handle.transform.localRotation = Quaternion.Euler(0, 0, handle_rot_angle);
            }
            Open_close();
        }
    }
